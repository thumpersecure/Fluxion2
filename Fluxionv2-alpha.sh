...
readonly FLUXIONRevision=13  # Incremented for fixes

# Declare window ratio bigger = smaller windows
FLUXIONWindowRatio=4

# Allow to skip dependencies if required, not recommended
FLUXIONSkipDependencies=0  # Fixed: Changed default to 0
...
# Fixed: Better xdpyinfo availability check
if ! command -v xdpyinfo >/dev/null 2>&1; then # Assure display probe.
...
# =================== < Library Includes > =================== #
# ============================================================ #
# Fixed: Better error handling for missing libraries
...
while [ "$1" != "" ] && [ "$1" != "--" ]; do
  case "$1" in
    ...
    -i|--installer) FLUXIONSkipDependencies=0;;  # Fixed: Set to 0 for install mode
...
if [ -f "$FLUXIONPreferencesFile" ] && [ -x "$FLUXIONPreferencesFile" ]; then
  source "$FLUXIONPreferencesFile"
else
  mkdir -p "$(dirname "$FLUXIONPreferencesFile")"  # Fixed: Ensure directory exists
...
if [ -f "$FLUXIONPath/language/en.sh" ]; then
  ...
  echo -e "\\033[31mError: Default language file missing: $FLUXIONPath/language/en.sh\\033[0m"
  exit 8
fi
...
fluxion_startup() {
  ...
  iptables-save >"$FLUXIONIPTablesBackup" 2>/dev/null || {
    echo "Warning: Could not backup iptables rules"
  }
  ...
  sleep 0.1
  # Fixed: Better update check with error handling
  if command -v curl >/dev/null 2>&1; then
    ...
    if installer_utils_check_update "https://$fluxionDomain/$fluxionPath" \
      ...
      $FLUXIONVersion $FLUXIONRevision 2>/dev/null; then
      if installer_utils_run_update "https://$updateDomain/$updatePath" \
        "FLUXION-V$FLUXIONVersion.$FLUXIONRevision" "$FLUXIONPath" 2>/dev/null; then
        fluxion_shutdown
      fi
    fi
  fi
  ...
  # Fixed: Updated dependency list and better error handling
  ...
  # Fixed: Only check dependencies if not skipping them
  if [ "$FLUXIONSkipDependencies" != "1" ]; then
    ...
    if [ $FLUXIONMissingDependencies -eq 1 ]; then
        echo -e "\n\n"
    ...
    echo -e "\n\n"
    format_center_literals "[ ${CSYel}Dependency check skipped${CClr} ]"
    echo -e "$FormatCenterLiterals"; sleep 2
  fi
...
fluxion_shutdown() {
  ...
  local processes
  readarray processes < <(ps -A 2>/dev/null || ps aux)
  ...
  for targetID in "${targets[@]}"; do
    ...
    local targetPID
    targetPID=$(
      echo "${processes[@]}" | awk '$4~/'"$targetID"'/{print $1}' 2>/dev/null
    )
  ...
  # Fixed: Better variable check for authService
  ...
  if [ ! $FLUXIONDebug ]; then
    ...
    sandbox_remove_workfile "$FLUXIONWorkspacePath/*" 2>/dev/null || true
  fi

  if [ $FLUXIONWIKillProcesses ]; then
    echo -e "$CWht[$CRed-$CWht] $FLUXIONRestartingNetworkManagerNotice$CClr"

    # Fixed: Better network manager detection and restart
    if command -v systemctl >/dev/null 2>&1; then
      ...
      # Also restart systemd-resolved if present
      if systemctl is-active --quiet "systemd-resolved" 2>/dev/null; then
        systemctl restart "systemd-resolved" &> $FLUXIONOutputDevice &
      fi
    elif command -v service >/dev/null 2>&1; then
      # Try different service names for older systems
      for service in "network-manager" "networking" "networkmanager"; do
        service "$service" restart &> $FLUXIONOutputDevice & 2>/dev/null || true
      done
    fi
  fi
...
# ================== < Helper Subroutines > ================== #
# ============================================================ #
# The following will kill the parent process & all its children.
fluxion_kill_lineage() {
  ...
  # Fixed: Better process killing with error handling
...
fluxion_handle_target_change() {
  ...
  local targetInfo
  readarray -t targetInfo < <(cat "$FLUXIONWorkspacePath/target_info.txt" 2>/dev/null || echo "")

  if [ ${#targetInfo[@]} -ge 3 ]; then
...
fluxion_set_resolution() { # Windows + Resolution
  ...
  # Get dimensions
  # Fixed: Better screen size detection
  if command -v xdpyinfo >/dev/null 2>&1; then
    SCREEN_SIZE=$(xdpyinfo 2>/dev/null | grep dimension | awk '{print $4}' | tr -d "(")
  else
  ...
  if command -v bc >/dev/null 2>&1; then
    PROPORTION=$(echo "scale=2; $SCREEN_SIZE_X/$SCREEN_SIZE_Y" | bc)
    NEW_SCREEN_SIZE_X=$(echo "scale=0; $SCREEN_SIZE_X/$FLUXIONWindowRatio" | bc)
    NEW_SCREEN_SIZE_Y=$(echo "scale=0; $SCREEN_SIZE_Y/$FLUXIONWindowRatio" | bc)

    NEW_SCREEN_SIZE_BIG_X=$(echo "scale=0; 1.5*$SCREEN_SIZE_X/$FLUXIONWindowRatio" | bc)
    NEW_SCREEN_SIZE_BIG_Y=$(echo "scale=0; 1.5*$SCREEN_SIZE_Y/$FLUXIONWindowRatio" | bc)

    SCREEN_SIZE_MID_X=$(echo "scale=0; $SCREEN_SIZE_X + ($SCREEN_SIZE_X - 2 * $NEW_SCREEN_SIZE_X) / 2" | bc)
    SCREEN_SIZE_MID_Y=$(echo "scale=0; $SCREEN_SIZE_Y + ($SCREEN_SIZE_Y - 2 * $NEW_SCREEN_SIZE_Y) / 2" | bc)
    ...
    # Fallback geometry if bc is not available
    TOPLEFT="-geometry 80x24+0+0"
    TOPRIGHT="-geometry 80x24-0+0"
    TOP="-geometry 80x24+400+0"
    BOTTOMLEFT="-geometry 80x24+0-0"
    BOTTOMRIGHT="-geometry 80x24-0-0"
    BOTTOM="-geometry 80x24+400-0"
    LEFT="-geometry 80x24+0+300"
    RIGHT="-geometry 80x24-0+300"
    TOPLEFTBIG="-geometry 120x36+0+0"
    TOPRIGHTBIG="-geometry 120x36-0+0"
  fi
}
...
fluxion_do() {
  ...
  # Notice, the instruction will be added to the Do Log
...
fluxion_undo() {
  ...
  for (( __fluxion_undo__i=${#__fluxion_undo__history[@]}; \
    ...
    if eval ${__fluxion_undo__namespace}_${FLUXIONUndoable["$__fluxion_undo__command"]}_$__fluxion_undo__identifier; then
      echo "Undo-chain succeeded." > $FLUXIONOutputDevice
...
fluxion_unset_language() {
  ...
  if [ "$FLUXIONPreferencesFile" ]; then
    sed -i.backup "/FluxionLanguage=.\+/ d" "$FLUXIONPreferencesFile" 2>/dev/null || true
  fi
}
...
fluxion_set_language() {
  if [ ! "$FluxionLanguage" ]; then
    ...
    local languageCodes
    if [ -d "$FLUXIONPath/language" ]; then
      readarray -t languageCodes < <(ls -1 "$FLUXIONPath/language" | sed -E 's/\.sh//')
    else
    ...
    readarray -t languages < <(
      head -n 3 "$FLUXIONPath/language"/*.sh 2>/dev/null |
      grep -E "^# native: " |
      sed -E 's/# \w+: /'
    )
  ...
  find -type d -name language 2>/dev/null | while read language_dir; do
  ...
  if [ "$FLUXIONPreferencesFile" ]; then
    if grep -q "FluxionLanguage=.\+" "$FLUXIONPreferencesFile" 2>/dev/null; then
...
fluxion_allocate_interface() { # Reserve interfaces
  ...
  if interface_is_wireless $identifier; then
    ...
    if [ "$FLUXIONWIKillProcesses" ]; then
      ...
      # Fixed: Better process detection and killing
      if command -v airmon-ng >/dev/null 2>&1; then
        ...
        local conflicting_processes=$(airmon-ng check 2>/dev/null | awk 'NR>6{print $2}' | grep -v "^$")
        for program in $conflicting_processes; do
          if [ -n "$program" ]; then
            killall "$program" &> $FLUXIONOutputDevice || true
          fi
        done
      else
        # Fallback: kill common conflicting processes
        for program in "NetworkManager" "wpa_supplicant" "dhclient"; do
          killall "$program" &> $FLUXIONOutputDevice 2>/dev/null || true
        done
      fi
    fi
  ...
  # If using airmon-ng, let airmon-ng rename the interface.
  ...
  if [ $allocatingWirelessInterface ]; then
    ...
    if [ $FLUXIONAirmonNG ]; then
      local -r newIdentifier=$(
        airmon-ng start $identifier 2>/dev/null |
        grep "monitor .* enabled" |
        grep -oP "wl[a-zA-Z0-9]+mon|mon[0-9]+|prism[0-9]+" | head -n1
      )
    else
...
fluxion_target_get_candidates() {
  ...
  # Fixed: Better xterm execution with fallback
  local xterm_cmd="airodump-ng -Mat WPA ${2:+--channel $2} ${3:+--band $3} -w $FLUXIONWorkspacePath/dump $1"
  
  if ! xterm -title "$FLUXIONScannerHeader" $TOPLEFTBIG \
      -bg "#000000" -fg "#FFFFFF" -e "$xterm_cmd" 2> $FLUXIONOutputDevice; then
  ...
  # Fixed: Better MAC address matching pattern
  local -r matchMAC="([A-Fa-f0-9][A-Fa-f0-9]:){5}[A-Fa-f0-9][A-Fa-f0-9]"
...
fluxion_get_target() {
  ...
  # Abort if errors occurred while searching for candidates.
  ...
  for candidateAPInfo in "${FluxionTargetCandidates[@]}"; do
    ...
    candidatesESSID[i]=$(eval "echo \$sanitizedESSID'" 2>/dev/null || echo "$sanitizedESSID")
...
fluxion_target_normalize_SSID() {
  ...
  # shit'll hit the fan and we'll have an extremely distressed user.
  # Replacing ' ', '/', '.', '~', '\' with '_'
...
fluxion_target_tracker_daemon() {
  ...
  # Save/overwrite the new target information to the workspace for retrieval.
...
fluxion_target_tracker_stop() {
  ...
  kill -s SIGABRT $FluxionTargetTrackerDaemonPID &> /dev/null || true
...
fluxion_target_set() {
  # Check if attack is targeted & set the attack target if so.
  ...
  if [ \
    ...
    # If we've got a candidate target, ask user if we'll keep targeting it.
...
fluxion_set_attack() {
  ...
  local attacks
  if [ -d "$FLUXIONPath/attacks" ]; then
    readarray -t attacks < <(ls -1 "$FLUXIONPath/attacks")
  else
    echo "Error: Attacks directory not found"
    return 1
  fi
  ...
  for attack in "${attacks[@]}"; do
    local identifier=""
    if [ -f "$FLUXIONPath/attacks/$attack/language/$FluxionLanguage.sh" ]; then
      identifier=$(
...
fluxion_unprep_attack() {
  ...
  # Remove any lingering targeting subroutines loaded.
  unset attack_targetting_interfaces 2>/dev/null || true
  unset attack_tracking_interfaces 2>/dev/null || true

  # Remove any lingering restoration subroutines loaded.
  unset load_attack 2>/dev/null || true
  unset save_attack 2>/dev/null || true
...
fluxion_run_attack() {
  ...
  # Fixed: Better systemd-resolved restart logic
  if command -v systemctl >/dev/null 2>&1; then
    if systemctl is-active --quiet systemd-resolved 2>/dev/null; then
      systemctl restart systemd-resolved.service &>/dev/null || true
    fi
  elif command -v service >/dev/null 2>&1; then
    service systemd-resolved restart &>/dev/null 2>&1 || true
  fi
